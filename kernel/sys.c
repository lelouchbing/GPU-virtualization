...
/* sangyan */
SYSCALL_DEFINE3(addrmap, unsigned long __user *, pfn, unsigned long, len, unsigned long __user *, hva)
{
        unsigned long hva_value;
        unsigned long s2;
        unsigned long pfn_value[1000];
...
//whb
SYSCALL_DEFINE0(delay)
{
	
	// printk("syscall here\n");
	// printk("ppid: %ld\n", ppid);
	// struct pid* hostpid = NULL;
	// hostpid = find_get_pid((int)ppid);
...
//now use current 
	...
	printk("syscall_delay start\n");

	if(!host_task)
	{
		printk("syscall delay : NULL current struct\n");
		return -1;
	}
	...
	// host_task->mm->is_addrmap_mm == 0;
		...
		// //reset pgd regs , so this task come back to gvirtus-backend
	// //TODO:
	// //		regs,  setg wake mach
	...
	host_task->mm->pgd = host_task->mm->origin_pgd;
	flush_tlb_all();
...
SYSCALL_DEFINE2(sleepgvirtus, unsigned long , addr, unsigned long , size)
{
	...
	if(!host_task)
	{
		printk("syscall sleepgvirtus : NULL current struct\n");
		return -1;
	}
	printk("syscall sleepgvirtus: host pid: %d\n", host_task->pid);
	host_task->mm->is_addrmap_mm = 1;

	// store the addresses of backend cuda api into plt_cuda_address
	host_task->mm->plt_cuda_address = kmalloc(size, GFP_KERNEL);
	copy_from_user(host_task->mm->plt_cuda_address, (void __user*) addr, size);

	while(host_task->mm->is_addrmap_mm == 1)
	{
		//if not waken up by kvm kernel process, then schedule
		// if(host_task->state == 0)
		// 	printk("gvirtus-backend subprocess is running\n");
		//set_task_state(host_task, TASK_INTERRUPTIBLE);
		schedule();
	}
	//host_task->mm->is_addrmap_mm = 0;
	host_task->mm->is_addrmap_mm = 1;
	
	printk("syscall pgd: %lx\n", virt_to_phys(host_task->mm->pgd));
