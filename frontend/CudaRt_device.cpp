...
/*
    ...
    {
    	Frontend::GetFrontend()->delay = 0;

    	CudaRtFrontend::Prepare();
	vector<char*> tempbuf = Frontend::GetFrontend()->GetInputBuffer()->delayBuffer;
	vector<size_t> tempbufsize = Frontend::GetFrontend()->GetInputBuffer()->delayBufferSize;

	for(unsigned int i = 0; i < tempbuf.size(); ++i)
	{
		Frontend::GetFrontend()->GetInputBuffer()->Add(tempbufsize[i]);
		Frontend::GetFrontend()->GetInputBuffer()->Add(tempbuf[i],tempbufsize[i]);
	}
    ...
    else
    {
    	printf("delay start \n");
    	CudaRtFrontend::Prepare();
	Frontend::GetFrontend()->GetInputBuffer()->delayBuffer.clear();
	Frontend::GetFrontend()->GetInputBuffer()->delayBufferSize.clear();
    	Frontend::GetFrontend()->delay = 1;
	}

    return CudaRtFrontend::GetExitCode();
*/
    	CudaRtFrontend::Prepare();
	Frontend::GetFrontend()->iskvmdelay = 1 - Frontend::GetFrontend()->iskvmdelay;
	return cudaSuccess;
}
