...
CUDA_ROUTINE_HANDLER(Memcpy) {
    ...
    switch (kind) {
        ...
        case cudaMemcpyDeviceToHost:
            ...
            dst = new char[count];
	    host_dst_addr = input_buffer->BackGet<size_t>();
            /* skipping a char for fake host pointer */
            //input_buffer->Assign<char>();
            src = input_buffer->GetFromMarshal<void *>();

            exit_code = cudaMemcpy(dst, src, count, kind);
            printf("src: %x, size: %d\n", (unsigned int)src, sizeof(src));
            printf("res:%d\n", exit_code);

	    if(delay == true)
	    {
	    	//record local mem
	    	memmap.insert(make_pair<size_t,char*>(host_dst_addr,(char*)dst));
            	result = new Result(exit_code);
	    }
	    else
	    {
