...
public:
    ...
    Result * Execute(std::string routine, Buffer * input_buffer, bool delay);
	// TODO(DEBUG): return original cuda api address:
	unsigned long* GetOriginalAddress_DEBUG();
...
#define CUDA_ROUTINE_HANDLER(name) Result * handle##name(CudaRtHandler * pThis, Buffer * input_buffer, bool delay)
