...
void debug_show_host_regs(struct task_struct *task)
{
	struct pt_regs * regs;
...
EXPORT_SYMBOL_GPL(debug_show_host_regs);


struct pt_regs kvm_fill_regs(struct task_struct * task, struct pt_regs para_regs)
{
...
EXPORT_SYMBOL_GPL(kvm_fill_regs);

void kvm_fill_back_regs(struct task_struct * task, struct pt_regs origin)
{
	struct pt_regs * regs;
	regs = task_pt_regs(task);
	*regs = origin;
				...
				unsigned long cuda_api_list_gva)
{
	...
	*/
	...
	{
		printk("error : kvm_pgd_alloc is NULL\n");
		return -1;
	...
	//debug
	hmm->pgd = hmm->kvm_alloc_pgd(hmm);
	
	if(!hmm->pgd)
	{
		printk("error: pgd alloc error\n");
		return -1;
	}
	...
	*/
	...
	if(!wake_up_host(host_task))
	{
		printk("host_task state: is running \n");
		//now use is_addrmap to wake up gvirtus_backend
	}
	else
	{
		printk("host_task state: wake up!!!!\n");
		
	}
	while(host_task->mm->is_addrmap_mm == 0)
		schedule();

	//do not return until backend-gvritus finished 
	
	// int start;
	// unsigned long esp_start = regs.sp & 0xfffff000;
 //    unsigned long espp = regs.sp & 0xfff;
    ...
    //pass
    // unsigned long* addr = ioremap(esp_add, 1024);
    // int i;
    // for(i = 0; i < 1024; i++)
    // {
    //         printk("addr: %lx val: %lx ", (unsigned long)(esp_add + 4 * i), *(addr + i));
    //         if((unsigned long)addr % 32 == 28)
    //         	printk("\n");
    // }
    // iounmap(addr);
	//mark gvirtus backend process
	...
	while(1)
	{
		switch(host_task->mm->is_addrmap_mm)
		{
			case 1:
				...
				break;
			case 0:
				//backend process running success
				goto running_end;
				break;
			case 2:
...
// 	for(check_s = back_regs->sp; check_s <= back_regs->sp+128; check_s += 4)
// 	{
// 		printk("addr: %lx, val : %lx\n", check_s, (unsigned long)*(unsigned long*)(check_s));
// 	}
...
int kvm_emulate_hypercall(struct kvm_vcpu *vcpu)
{
	...
	//whb
	unsigned long vm_eip, vm_esp;
	vm_eip = kvm_register_read(vcpu, VCPU_REGS_RIP);
	vm_esp = kvm_register_read(vcpu, VCPU_REGS_RSP);
	...
	case KVM_HC_MMU_OP:
		...
		break;
	case KVM_HC_TEST:
		r = kvm_addrmap(vcpu, a0, a1, a2, a3);
		ret = r;
		break;
	default:
		ret = -KVM_ENOSYS;
