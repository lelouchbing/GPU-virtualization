...
unsigned long gfn_to_hva(struct kvm *kvm, gfn_t gfn)
{
	...
	if (!slot)
	{
                //printk(KERN_INFO "sangyan: slot null\n");
         	return bad_hva();
        }
...
gfn_t gfn_to_pfn(struct kvm *kvm, gfn_t gfn)
{
	struct page *page[1];
	unsigned long addr;
	int npages;
...
EXPORT_SYMBOL_GPL(wake_up_host);
//whb
...
gpa_t temp;
	...
	//unsigned long pgd_gfn;
	void* temp_pgd = NULL;
	//void* temp_pmd = kmalloc(4096, GFP_KERNEL);

	pgd_t walk_pgd;
	pmd_t walk_pmd;

	//for(start_addr = 0; start_addr < 0xc0000000; start_addr += 4096)
	start_addr = addr;
		pfn = 0;
		gfn = 0;
		//printk("fill loop start \n");
		//printk("addr index: %lx \n", pgd_index(start_addr));
		pgd = mm->guest_pgd + pgd_index(start_addr);

		if(!native_pgd_val(*pgd) || pgd_bad(*pgd))
			return -1;

		temp = (gpa_t) native_pgd_val(*pgd);
	 	gfn = temp >> PAGE_SHIFT;
	 	unsigned long* temp_addr;
	 	temp_addr = (unsigned long*)gfn_to_hva(vcpu->kvm, gfn);
	 	pfn = gfn_to_pfn(vcpu->kvm, gfn);

	 	//temp_addr = (unsigned long*)(gfn_to_hva(vcpu->kvm, gfn));
 		// if(kvm_is_error_hva(*temp_addr))
 		// {
	 	// 	//printk("error : %lx is error hva, gfn: %lx", temp_addr, gfn);
	 	// 	continue;
 		// }

	 //  	temp_pgd = kmalloc(4096, GFP_KERNEL);
 	//   	memset(temp_pgd, 0, 4096);
		// memcpy(temp_pgd, temp_addr, 4096);

		
	 	//printk("lvl1 val : %lx", *((unsigned long*)temp_addr));
	 	
	 	//temp_addr = (temp_addr + offset_in_page(temp));

	 	//printk("lvl1 val : %lx", temp_addr);

	 	//debug ok here

	 	//*pgd =native_make_pgd(pfn << 12 | (temp & 0xFFF));
	 	// walk_pgd = native_make_pgd((pfn << PAGE_SHIFT) | (temp & 0xFFF));
	 	// pgd = &walk_pgd;
	 	//pgd = temp_addr;

	 	//pgd_gfn = gfn;
	 	//printk("debug vadd: %lx", temp_addr);
	 	//return;
		if(native_pgd_val(*pgd) && !pgd_bad(*pgd))
		{
			/*

			pmd = pmd_offset(pgd, start_addr);
			temp = (gpa_t) native_pmd_val(*pmd);
			gfn = temp >> PAGE_SHIFT;
	 	...
	 	// 	memset(temp_pmd, 0, 4096);
			// memcpy(temp_pmd, temp_addr, 4096);
			...
			if(gfn != pgd_gfn)
			{
		 	...
		 	//temp_addr =   (temp_addr + offset_in_page(temp));
	 		//printk("\tlvl2 val : %lx", temp_addr);
	 		set_pud(&walk_pmd,(pfn << PAGE_SHIFT) | (temp & 0xFFF));
	 		pmd = &walk_pmd;
	 		//pmd = temp_addr;


	 		//debug ok here
	 		printk("start address: %lx\n", start_addr);
			...
			pmd = pgd;
			{
				//ptep = pte_offset_map(pmd, start_addr);
				ptep = temp_addr + pte_index(start_addr);
				...
				//if(ptep && pte_present(*ptep))
				{
					...
					// printk("pgd_vadd::% lx\n", ptep);
					// printk("pte val: %lx\n", *ptep);
					temp = native_pte_val(*ptep);
					gfn = temp >> PAGE_SHIFT;
					if(gfn == 0 || pte_none(*ptep))
						return -2;
					pfn = gfn_to_pfn(vcpu->kvm, gfn);
					//*ptep = native_make_pte(pfn << PAGE_SHIFT | temp & 0xfff);


					//find pte_t , copy to mm->pgd
					
					h_pte = kvm_get_pte(mm, start_addr);
					if(h_pte != NULL)
					{
						//*h_pte = *ptep;
						//pte_unmap(h_pte);
					//	h_pte = NULL;
						...
						//printk("pgd:%lx pte_p addr%lx start:%lx\n",pgd, ptep, start_addr);
						//printk("pgd val: %lx\n", *pgd);
						//printk("pte val: %lx\n", *ptep);
					...
					//else
					//	printk("error : null host pte\n");
				}
				//else printk("error pte\n");
			}	
			//else printk("error pmd\n");
		}		
...
void debug_kvm_showpt(struct mm_struct* mm)
{
	...
	for(; start < 0xc0000000; start += 4096)
	{
...
EXPORT_SYMBOL_GPL(debug_kvm_showpt);
...
EXPORT_SYMBOL_GPL(debug_get_phy);


gfn_t gfn_to_pfn_sy(struct kvm *kvm, gfn_t gfn, int pid)
{
	struct page *page[1];
	unsigned long addr;
	int npages;
...
EXPORT_SYMBOL_GPL(gfn_to_pfn_sy);
...
/* sangyan */
static long kvm_gfn_to_pfn(long arg)
{
        struct kvm *kvm = NULL;
	void * argp = (void *)arg;
	unsigned long gfn[500];
        unsigned long pfn[500];
        int r;
	int i, j;
        int ncopy, l;
		unsigned long mygfn, mypfn;

	struct kvm_gfn_translation kvm_gfn_trans;
        r = -EFAULT;
        ...
        list_for_each_entry(kvm, &vm_list, vm_list)
	{
	   ...
	   if(i == kvm_gfn_trans.vm_id) break;
		...
		// printk(KERN_INFO "sangyan: i %d pfn %lu\n", i, pfn[i]);
	     ...
	     r = -EFAULT;
             if (copy_to_user((unsigned long *)(mypfn), pfn, ncopy*sizeof(unsigned long))) goto out;
             mygfn = mygfn + ncopy*sizeof(unsigned long);
			 mypfn = mypfn + ncopy*sizeof(unsigned long);
        }
