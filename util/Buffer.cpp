...
{
/*
	size_t buffersize = mLength;
	
	char* temp = new char[mLength + sizeof(size_t)];
	memmove(temp, (char*) &buffersize,sizeof(size_t));
	memmove(temp+sizeof(buffersize),mpBuffer,mLength);
	*/
	char* temp = new char[mLength];
	memmove(temp,mpBuffer,mLength);
	delayBuffer.push_back(temp);
	delayBufferSize.push_back(mLength);
}
